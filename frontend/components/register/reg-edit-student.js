...
const RegEditStudent = () => {
  ...
  useEffect(() => {
    if (studentName) {
      ...
      if (parts[1]) {
        ...
        if (nameParts.length > 1) {
          ...
          setStudentExtension(nameParts[nameParts.length - 1] || "");
        }
      }
    }
    if (paramGender) {
    ...
    if (paramDateOfBirth) {
      const dateParts = paramDateOfBirth.split(" - ");
  ...
  }, [studentName, paramGender, paramDateOfBirth]);
